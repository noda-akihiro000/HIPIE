...
class HIPIE_IMG(nn.Module):
    ...
    def forward(self, batched_inputs, do_postprocess=True):
        ...
        if self.training:
            ...
            if self.parallel_det:
                language_dict_features = self.forward_text(captions, device=self.device, task=task)
            else:
                language_dict_features = self.forward_text(captions, device=self.device)
            ...
            for k in loss_dict.keys():
                if self.detr.ensemble_maskdino and '_maskdino' in k: # hack, do not drop mask dino loss
                    continue
                if k in weight_dict:
                    if task == "detection":
    ...
    def inference(self, box_cls, box_pred, mask_pred, image_sizes, positive_map_label_to_token, num_classes, score_thres=0.0, task=None, iou_pred=None,is_thing=None,sizes=None,output=None,
        ...
        if self.detr.ensemble_maskdino:
        ...
        for i, (logits_per_image, box_pred_per_image, image_size, iou_per_image) in enumerate(zip(
            ...
            if task == "detection":
                ...
                if self.detr.ensemble_maskdino and self.detr.mask_dino_fixed_linear_head:
